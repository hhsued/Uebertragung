    ...
    q-stepper(
      ...
      q-step(
        ...
        div(v-if="!Konfig.WebSockets.installiert")
          ...
          q-btn(label="Programm beenden", color="primary", unelevated, @click="on_Beenden")
        div(v-if="Konfig.WebSockets.installiert")
          div Habe die benötigte Erweiterung 'WebSockets' gefunden. Wir können weiter machen!
          q-stepper-navigation
            q-btn(@click="Schritt = 3" color="primary" label="Weiter")
      q-step(
        :name="3"
        title="OBS"
        caption="Profil"
        ...
        div(v-if="!Weiter")
          ...
          br
          br
          q-input(v-model="Konfig.Profil.Name", label="Name des neuen Profils", hint="Bitte keine Leerzeichen und Sonderzeichen nutzen. Sollten Leerzeichen vorhanden sein, werden diese entfernt, ebenso Sonderzeichen")
          q-btn(label="Neues Profil anlegen", @click="on_Neues_Profil", v-if="Konfig.Profil.Name.length > 2")
        div(v-if="Weiter")
