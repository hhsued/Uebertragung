    ...
    q-card(style="min-width: 400px")
      q-card-section
        q-toolbar
          ...
          q-btn(icon="add", unelevated, @click="on_neue_Person")
      q-card-section
        q-select(v-model="Filter", :options="Filterwerte", label="Filter", @input="on_Filtern")
      q-card-section(v-if="bearbeiten")
        q-toolbar
          q-space
          q-btn(icon="close", unelevated, @click="bearbeiten = false")
        ...
        q-select(label="Bereich", :options="Filterwerte", v-model="Bereich")
          template(v-slot:after)
            q-btn(icon="add", unelevated, @click="on_neuer_Bereich")
        ...
        q-list(separator)
          q-item(v-for="(Person, Index) in gefilterte_Personen", :key="Index")
            q-item-section(avatar)
              q-btn(icon="delete", unelevated, @click="on_loeschen(Index)")
              q-btn(icon="edit", unelevated, @click="on_bearbeiten(Index)")
            q-item-section
              | {{Person}}
</template>
<script>
...
export default {
  ...
  methods: {
    ...
    on_bearbeiten (Index) {
      this.Index = Index
      const lobjPersonen = JSON.parse(gmodDS.readFileSync(this.$store.state.Personen.Datenpfad, 'utf-8'))
      this.Vorname = lobjPersonen[Index].Vorname
      this.Nachname = lobjPersonen[Index].Nachname
      this.Bereich = lobjPersonen[Index].Bereich
      this.Amt = lobjPersonen[Index].Amt
      this.Zusatz = lobjPersonen[Index].Zusatz
      this.bearbeiten = true
    },
    on_loeschen (Index) {
      this.$q.dialog({
