  ...
  .q-gutter-y-md
    q-dialog(
      ...
      q-card
        q-card-section
          q-toolbar
            q-toolbar-title
            q-btn(icon="close", unelevated, v-close-popup, dense)
            q-btn(
              icon="done",
              unelevated,
              @click="on_Daten_speichern",
              v-if="Liederart !== null && Object.keys(Strophen).length > 0",
              dense
            )
    ...
    q-toolbar
      q-toolbar-title
        | Vorgaben
      q-space
      q-btn(icon="add", unelevated, @click="on_neue_Vorgabe", dense)
    div(v-if="Object.keys(Dateien).length > 0")
      q-list(separator)
        q-expansion-item(
          v-for="(Datei, Index) in Object.keys(Dateien)",
          :key="Index",
          expand-separator,
          switch-toggle-side,
          expand-icon-toggle,
            ...
            q-item-section(side)
              .row.items-center
                q-btn(
                  icon="add",
                  unelevated,
                  @click="on_neues_Lied(Datei)",
                  dense
                )
                q-btn(icon="delete", unelevated, dense)
                q-btn(
                  icon="save",
                  unelevated,
                  @click="on_speichern(Datei)",
                  dense
                )
          q-card
            q-card-section
              q-list
                q-item(v-for="(Lied, LIndex) in Dateien[Datei]", :key="LIndex")
                  q-item-section(avatar)
                    q-btn(icon="delete", unelevated, dense)
                  q-item-section(avatar)
                    q-btn(icon="edit", unelevated, dense)
                  q-item-section
                    .text-h6 {{ Lied.Art }}
                    b Lied-Nr.: {{ Lied.Lied }}
