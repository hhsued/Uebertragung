...
q-card-section
  q-dialog(v-model="Dialog")
    q-card(style="min-width: 450px")
      q-card-section
        div(class="text-h6")|Standardperspektive:&nbsp;{{Perspektivenname}}
      ...
      q-card-section(class="q-pt-none")
        ...
        q-list(bordered, separator)
          ...
          q-item
            ...
            q-item-section
              div(v-for="(Mikro, MAIndex) in Perspektivdaten.aktiv.Mikros", :key="MIndex")  {{Mikro}}
          q-item
            q-item-section Quellen
            q-item-section
              div(v-for="(Quelle, QAIndex) in Perspektivdaten.aktiv.Quellen", :key="QAIndex")  {{Quelle}}
      q-card-section(class="q-pt-none")
        b Inaktive Quellen
        q-list(bordered, separator)
          q-item
          ...
          q-item
            q-item-section Mikrofone
            q-item-section
              div(v-for="(Mikro, MAIndex) in Perspektivdaten.inaktiv.Mikros", :key="MIndex")  {{Mikro}}
          q-item
            q-item-section Quellen
            q-item-section
              div(v-for="(Quelle, QAIndex) in Perspektivdaten.inaktiv.Quellen", :key="QAIndex")  {{Quelle}}
  ...
  .row(style="margin-left:5px;margin-right:5px; border: 1px solid")
    ...
    .col(:class="Perspektive ? 'bg-green-5 text-white' : 'bg-red-5 text-white'", @click="on_Modus", style="cursor: pointer")
      .text-h6.text-center {{JaNein}}
    .col-1(v-if="GlobalerSpeicher !== ''")
      q-btn.full-width(icon="help_outline", unelevated, @click="Dialog = !Dialog")
</template>
<script>
