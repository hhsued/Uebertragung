...
const lobjKomponentenPfad = gmodPath.join(process.cwd(),
  'Daten', 'Einstellungen', 'Komponenten')
const lobjPfad = gmodPath.join(process.cwd(),
  'Daten', 'Einstellungen')
...
export default {
  pruefe_auf_Datenverzeichnis () {
    if (!gmodFS.existsSync(gmodPath.join(process.cwd(),
      'Daten'))) {
      return false
      /* alert('Das Verzeichnis mit den Einstellungen und den Daten ist nicht vorhanden, Programm kann nicht gestartet werden')
