...
export default {
  ...
  methods: {
    async YouTube() {
      ...
      this.$Log.Schreibe("Habe die anstehenden YouTube-Übertragungen geholt");
      if (lobjUebertragungen === null) {
      } else {
        lobjUebertragungen.forEach(lobjEintrag => {
          larrAuswahl.push({
            label: lobjEintrag.snippet.title,
            value: lobjEintrag.id
          });
        });
        this.$q
          .dialog({
          ...
          .onOk(lstrID => {
            this.$Log.Schreibe(
              "YouTube-ID " + lstrID + " vom Benutzer gewählt"
            );
    ...
    GitHub() {
      if (this.$store.state.app.GitHub === null) {
        this.$Log.Schreibe("Prüfe auf aktiven Zugang zum GitHub");
        const lstrBenutzerverzeichnis = require("os").homedir();
        if (
          !gmodDS.existsSync(gmodPfad.join(lstrBenutzerverzeichnis, ".github"))
        ) {
          this.$q
            .dialog({
              title: "Aktualisierungen",
              message:
                "Die Aktualisierungen sind nicht aktiviert. Du brauchst dafür einen Zugangscode. Soll dieses jetzt gemacht werden?",
              prompt: {
                model: "",
                type: "text" // optional
              },
            ...
            .onOk(GitHubToken => {
              this.$Log.Schreibe(
                "GitHub-Schlüssel '" + GitHubToken + "' vom Benutzer eingegeben"
              );
              gmodDS.writeFileSync(
                gmodPfad.join(lstrBenutzerverzeichnis, ".github"),
                GitHubToken,
                "utf-8"
              );
          ...
          this.$store.commit("app/setze", {
            GitHub: gmodDS.readFileSync(
              gmodPfad.join(lstrBenutzerverzeichnis, ".github"),
              "utf-8"
            )
          });
          ...
          this.$q
            .dialog({
              ...
              message:
                ...
                ", neue Version: " +
                lobjGitHubDaten.data.tag_name,
              cancel: true,
              persistent: true
            })
            .onOk(() => {
              this.$q.notify(
                "Das Herunterladen hat im Hintergrund begonnen. Nach dem Abschluss kommt eine Nachricht hoch!"
              );
              this.neueVersion_laden(
                lobjGitHubDaten.data.assets[0].browser_download_url,
                gmodPfad.join(
                  process.cwd(),
                  lobjGitHubDaten.data.tag_name + ".zip"
                )
              );
            });
        } else {
      ...
      ldaDatei.on("finish", () => {
        ...
        this.$q
          .dialog({
            ...
            message:
              'Du musst dieses Programm beenden und die ZIP-Datei im Verzeichnis "' +
              process.cwd() +
              '" entpacken. Programm jetzt beenden?',
            cancel: true,
            persistent: true
          })
          .onOk(() => {
    ...
    erneute_Pruefung() {
      ...
      this.$Log.Schreibe(
        "Erneute Prüfung auf OBS manuell vom Benutzer angestoßen"
      );
      lobjFenster.reload();
    },
    on_Ende() {
    ...
    OBS_installiert() {
      switch (gmodBS.platform()) {
        ...
        case "win32":
          return gmodDS.existsSync(
            gmodPfad.join(gmodBS.homedir(), "AppData", "Roaming", "obs-studio")
          );
      }
